...
def worker_map(args_tuple):
    path, n_features, p_set, save_path = args_tuple
    points = np.fromfile(path, dtype=np.float32)
    points = points.reshape((-1, n_features))
    points, _, _ = simulate_fog(p_set, points, 10)
...
if __name__ == '__main__':
    ...
    for lidar_folder in LIDAR_FOLDERS:
        ...
        if not os.path.exists(src_folder):
                print(f"ERREUR : Dossier introuvable -> {src_folder}")
                continue
                
        all_files = [f for f in os.listdir(src_folder) if f.endswith('.bin')]
        all_files = sorted(all_files)
        all_paths = [os.path.join(src_folder, f) for f in all_files]
            
        print(f"-> BINGO ! J'ai trouvé {len(all_files)} fichiers à traiter.")
        if len(all_files) == 0:
                continue
        for available_alpha in available_alphas:
            ...
            # Préparation des arguments pour Windows
            worker_args = []
            for i in range(n):
                lidar_save_path = os.path.join(dst_folder, all_files[i])
                worker_args.append((all_paths[i], args.n_features, parameter_set, lidar_save_path))

            with mp.Pool(args.n_cpus) as pool:
                l = list(tqdm(pool.imap(worker_map, worker_args), total=n))